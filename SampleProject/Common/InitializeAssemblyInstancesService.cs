        ...
        public static void Initialize(Container container, Lifestyle lifestyle, Assembly assembly)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                if (interfaces.Count == 1)
                {
                    ...
                    {
                        if (type.Name == "InMemoryRepository`1" || type.Name == "Repository`1")
                        {
                            container.RegisterConditional(serviceType.GetGenericTypeDefinition(), type.GetGenericTypeDefinition(), c => c.Consumer.ImplementationType.Name.StartsWith(type.Name));
                        }
                        else
                        {
